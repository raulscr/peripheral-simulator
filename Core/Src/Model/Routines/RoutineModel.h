...
class RoutineModel {
public:
	explicit RoutineModel(const std::list<std::shared_ptr<model::EventModelInterface>>& routines = {}, uint32_t reloadTime = 0);
	virtual ~RoutineModel();

	void setReloadTime(uint32_t reloadTime);
	uint32_t getReloadTime() const;

	void setRoutines(const std::list<std::shared_ptr<model::EventModelInterface>>& routines);
	std::list<std::shared_ptr<model::EventModelInterface>> getRoutines() const;

	uint32_t getLastExecutionTime() const;
	void setLastExecutionTime(uint32_t lastExecutionTime);

private:
	std::list<std::shared_ptr<model::EventModelInterface>> m_routines;
